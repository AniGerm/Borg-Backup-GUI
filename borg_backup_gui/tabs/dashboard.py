...
class DashboardTab:
    ...
    def _build_ui(self):
        ...
        profile_frame.grid(row=0, column=0, sticky='ew', padx=12, pady=(12, 0))
        ...
        status_frame.grid(row=2, column=0, sticky='nsew', padx=12, pady=8)
        ...
        action_frame.grid(row=3, column=0, sticky='ew', padx=12, pady=8)
        ...
        ttk.Button(action_frame, text='Neues Profil...', command=self._on_new_profile).pack(side='left', padx=8, pady=8)

        stats_frame = ttk.LabelFrame(tab, text='Backup-Statistik')
        stats_frame.grid(row=4, column=0, sticky='ew', padx=12, pady=8)
        ...
        tray_frame.grid(row=5, column=0, sticky='ew', padx=12, pady=(0, 12))
        ...
        tray_check = ttk.Checkbutton(tray_frame, text='Tray-Symbol aktivieren', variable=self.tray_enabled_var,
        ...
        ttk.Button(tray_frame, text='Tray anwenden', command=self._on_tray_change).grid(row=0, column=3, padx=8, pady=8, sticky='w')
    ...
    def refresh(self):
        ...
        # Statistik
        files_label = str(status.last_files_count) if status.last_files_count else ('wird geladen...' if self._stats_missing(status) else 'noch keine Daten')
        source_label = status.last_source_size or ('wird geladen...' if self._stats_missing(status) else 'noch keine Daten')
        dedupe_label = status.last_dedupe_size or ('wird geladen...' if self._stats_missing(status) else 'noch keine Daten')
        item_label = status.last_item or ('wird geladen...' if self._stats_missing(status) else 'noch keine Daten')
    ...
    def _compute_traffic_light(self, profile, status):
        ...
        if self.app.backup_running:
            ...
            if last_success:
                ...
                if age <= 24:
                    color = '#28a745'; title = 'Backup aktuell und erfolgreich'
                elif age <= 48:
                    color = '#e0a800'; title = 'Backup erfolgreich, aber nicht mehr tagesaktuell'
                else:
                    color = '#dc3545'; title = 'Letztes erfolgreiches Backup ist zu alt'
...
import os  # noqa: E402 – für is_root-Prüfung oben
